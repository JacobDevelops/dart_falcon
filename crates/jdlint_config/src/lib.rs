...
pub fn load_config(path: &Path) -> Result<JdlintConfig, ConfigError> {
...
///
/// 1. `start_dir/jdlint.json` — if exists, return it
/// 2. Walk parent dirs up to filesystem root looking for `.git` dir; when found, check `<git_root>/jdlint.json`
/// 3. `$HOME/.jdlint.json` (via `std::env::var("HOME")`)
/// 4. Return None if nothing found
pub fn find_config(start_dir: &Path) -> Option<PathBuf> {
    // 1. Check start_dir/jdlint.json
    let local_config = start_dir.join("jdlint.json");
    if local_config.exists() {
        return Some(local_config);
    }

    // 2. Walk parent dirs looking for .git, then check <git_root>/jdlint.json
    let mut current = start_dir;
    loop {
        let git_dir = current.join(".git");
        if git_dir.exists() {
            let config_at_git_root = current.join("jdlint.json");
    ...
    // 3. Check $HOME/.jdlint.json
    if let Ok(home) = std::env::var("HOME") {
        let home_config = PathBuf::from(home).join(".jdlint.json");
...
pub fn load_or_default(start_dir: &Path) -> JdlintConfig {
    match find_config(start_dir) {
        Some(path) => match load_config(&path) {
            Ok(cfg) => cfg,
            Err(e) => {
                eprintln!("warning: failed to load config from {}: {}", path.display(), e);
                JdlintConfig::default()
            }
        },
        None => JdlintConfig::default(),
    }
}
