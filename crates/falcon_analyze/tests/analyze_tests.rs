use falcon_analyze::{AnalyzeContext, Rule, RuleRegistry};
...
fn test_mock_rule_analyze_empty() {
    let rule = NoOpRule;
    let (program, _) = falcon_dart_parser::parse("void main() {}");
...
fn test_analyze_parallel_empty_files() {
    use falcon_analyze::analyze_parallel;
...
fn test_end_to_end_parse_analyze_serialize() {
    ...
    let (program, parse_errors) = falcon_dart_parser::parse(source);
    assert!(
