...
/// (`linter.rules` or `project.rules`), asserting each names a known rule placed
...
// under its section (file rules under `linter.rules`, project rules under
// `project.rules`), grouped by category. Keep it in lockstep with the registry
// so new rules can't silently miss the showcase — and so neither section leaks
// the other kind of rule.
#[test]
fn root_falcon_json_splits_file_and_project_rules() {
    ...
    let project_listed = listed_rules(&json["project"]["rules"], "project.rules");

    // Neither section may contain the other kind of rule.
    for name in &linter_listed {
        assert!(
            !meta_for(name).unwrap().project,
            "linter.rules must not list project rule `{name}`"
        );
    }
    for name in &project_listed {
        assert!(
            meta_for(name).unwrap().project,
            "project.rules must not list file rule `{name}`"
        );
    }

    // Every file rule appears under linter.rules; every project rule under
    // project.rules. No rule is listed in both.
    let missing_file: Vec<_> = RULE_METADATA
        .iter()
        .filter(|m| !m.project && !linter_listed.contains(m.name))
        .map(|m| m.name)
        .collect();
    assert!(
    ...
    let missing_project: Vec<_> = RULE_METADATA
        .iter()
        .filter(|m| m.project && !project_listed.contains(m.name))
        .map(|m| m.name)
        .collect();
    assert!(
        missing_project.is_empty(),
        "project rules missing from project.rules showcase: {missing_project:?}"
    );
    assert!(
        linter_listed.is_disjoint(&project_listed),
        "a rule is listed in both linter.rules and project.rules"
    );
    assert_eq!(
        linter_listed.len() + project_listed.len(),
        RULE_METADATA.len(),
