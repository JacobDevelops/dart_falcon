...
mod dcl {
    ...
    fn scan_stmt(stmt: &Stmt, diags: &mut Vec<Diagnostic>, ctx: &AnalyzeContext, cfg: &Cfg) {
        match stmt {
            ...
            Stmt::Yield(y) => scan_expr(&y.value, false, diags, ctx, cfg),
            Stmt::Assert(a) => {
                scan_expr(&a.condition, false, diags, ctx, cfg);
                if let Some(m) = &a.message {
                    scan_expr(m, false, diags, ctx, cfg);
                }
            }
            _ => {}
        }
    ...
    fn scan_expr(
        ...
        match expr {
            ...
            // Folded in from the pyramid twin: reach through cascades, records,
            // switch expressions, and postfix operators so literals there count.
            Expr::Cascade {
                object, sections, ..
            } => {
                scan_expr(object, in_const, diags, ctx, cfg);
            ...
            Expr::Record { fields, .. } => {
                for field in fields {
                    scan_expr(&field.value, in_const, diags, ctx, cfg);
                }
            }
            Expr::Switch { subject, arms, .. } => {
                scan_expr(subject, in_const, diags, ctx, cfg);
                for arm in arms {
                    if let Some(guard) = &arm.guard {
                        scan_expr(guard, in_const, diags, ctx, cfg);
                    }
                    scan_expr(&arm.body, in_const, diags, ctx, cfg);
                }
            }
            Expr::PostfixIncDec { operand, .. } => scan_expr(operand, in_const, diags, ctx, cfg),
            _ => {}
        }
    }
}
