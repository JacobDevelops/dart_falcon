...
This document describes the standardized format for test fixtures in the jdlint corpus, used by the `validate-rules` harness to verify diagnostic accuracy across all rule implementations.

## Directory Structure

All test fixtures are organized under `crates/jdlint_rules/tests/corpus/` with the following layout:
...
jdlint aims for semantic compatibility with upstream Dart linters. The following mappings apply when porting rules:
...
2. **Run Analysis**: Executes jdlint on each fixture file, collecting actual diagnostics
