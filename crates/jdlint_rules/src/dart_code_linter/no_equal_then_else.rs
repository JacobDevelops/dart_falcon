...
fn last_stmt_span(stmt: &Stmt) -> &Span {
    match stmt {
        Stmt::Block(b) => b.stmts.last().map(|s| last_stmt_span(s)).unwrap_or(&b.span),
        other => other.span(),
    }
}
    ...
    match stmt {
        Stmt::If(i) => {
            if let Some(else_branch) = &i.else_branch {
                let then_src = normalize(span_src(ctx.source, i.then_branch.span()));
                let else_src = normalize(span_src(ctx.source, else_branch.span()));
