...
fn test_find_config_none_when_no_local_or_git_config() {
    // Create a temp dir with no falcon.json and no parent .git
    let id = COUNTER.fetch_add(1, Ordering::Relaxed);
    let dir = std::env::temp_dir().join(format!("falcon_test_no_config_{}", id));
    fs::create_dir_all(&dir).expect("create temp dir");

    // Verify there's no local or git-root config (the only ambiguity is $HOME/.falcon.json)
    assert!(!dir.join("falcon.json").exists());

    let found = find_config(&dir);

    // If $HOME/.falcon.json doesn't exist we expect None; if it does the function
