# falcon Phase 1: Comprehensive Implementation Plan (Deliberate Consensus)
...
6. **Config-as-Contract**: falcon.json is the contract; each field and its default must be documented and testable. No magic or implicit behavior.
...
- **Crates**: falcon_dart_parser, falcon_syntax, falcon_analyze, falcon_rules, falcon_diagnostics, falcon_lsp, falcon_config, falcon_cli, falcon (binary)
- **Macros crate** (optional): falcon_analyze_macros if boilerplate becomes severe
...
- **M6**: Diff-based rule validation — run falcon and dart_code_linter side-by-side on corpus; flag mismatches
...
- **M7**: Full corpus profiling — `perf` profile falcon on jfit mobile; identify bottlenecks early
...
**What gets built:**
1. Rename repo from `falcon` to `falcon`; update Cargo.toml `name = "falcon"`
2. Scaffold workspace Cargo.toml with 9 crates listed
3. Create flake.nix for falcon with Rust stable toolchain + build target
4. Setup GitHub Actions CI: lint, test, bench (Linux x86_64 only for Phase 1)
5. Create `.omc/plans/falcon-rules-matrix.md` (rule-to-crate mapping)
6. Establish dev environment: nix flake (test `nix develop` produces falcon binary)
...
**Exit criteria:**
- Repo renamed to falcon; all references updated in git history
...
**What gets built:**
1. `.omc/docs/PARSER_GRAMMAR.md` — Dart 3.x grammar scope for falcon Phase 1
...
**What gets built:**
1. `.omc/docs/PARALLELISM_MODEL.md` — Rayon parallelization strategy for falcon
...
**What gets built:**
1. `falcon_dart_parser/src/lexer.rs` — token stream for Dart 3.x
   ...
   - Comment types: //, /*, */, ///
2. `falcon_dart_parser/src/grammar.md` — Dart grammar reference (transcribed from spec)
...
3. Test suite: `falcon_dart_parser/tests/lexer_tests.rs`
...
**What gets built:**
1. `falcon_dart_parser/src/parser.rs` — recursive descent parser
...
2. Parser tests: `falcon_dart_parser/tests/parser_tests.rs`
...
**What gets built:**
1. `falcon_syntax/src/lib.rs` — AST node definitions
...
2. `falcon_syntax/src/visitor.rs` — Visitor trait for AST traversal
...
3. Tests: `falcon_syntax/tests/ast_tests.rs`
...
**What gets built:**
1. Corpus test suite: `falcon_dart_parser/tests/corpus_tests.rs`
   - Compile jfit mobile lib (214 files) via falcon parser
...
- File: `crates/falcon_syntax/src/FORMAT.md`
- Content:
  ...
  - Lock constant: `const FALCON_AST_FORMAT_VERSION: &str = "1.0"` in falcon_syntax crate
- Acceptance:
  - FORMAT.md committed and reviewed by Architect
  - TRAIT_CONTRACTS approval gate (M0.5) includes AST format review
  - `FALCON_AST_FORMAT_VERSION` constant present in falcon_syntax/src/lib.rs
...
**What gets built:**
1. `falcon_diagnostics/src/lib.rs`
...
2. Tests: `falcon_diagnostics/tests/diag_tests.rs`
...
2. `falcon_analyze/src/visitor_macros.rs` (optional): derive macro for common visitor patterns
3. Tests: `falcon_analyze/tests/analyze_tests.rs`
...
**What gets built:**
1. `falcon_config/src/lib.rs`
   - Struct `FalconConfig`: loaded from falcon.json
   ...
   - Defaults: default falcon.json with all Phase 1 rules enabled
...
3. Tests: `falcon_config/tests/config_tests.rs`
   - Load valid falcon.json; verify parsing
...
1. CLI flag: `falcon check --config /path/to/custom.json` (explicit override)
2. Current working directory: `./falcon.json`
3. Git root directory: `<git-root>/falcon.json` (walk parent dirs until .git found)
4. User home directory: `$HOME/.falcon.json`
...
- Integration: test with no falcon.json present (defaults applied correctly)

**Tests written:**
- Unit: deserialize valid, invalid, partial configs
- Integration: real falcon.json files; verify fields
- Snapshot: config deserialization output

**Exit criteria:**
- falcon.json fully specified and validated
...
**What gets built:**
1. `falcon_tracing/src/lib.rs` or integration with `tracing` crate (optional separate crate)
...
2. CLI flag: `--verbose` (enables DEBUG logs; default INFO)
   - Examples: `falcon check . --verbose` shows detailed analysis traces
...
4. Tests: `falcon_cli/tests/logging_tests.rs`
...
**What gets built:**
1. `falcon_cli/src/lib.rs` — CLI argument parsing
...
2. `falcon/src/main.rs` — binary entrypoint
   - Parse args, load config, run selected subcommand
   - Exit with appropriate code
3. Tests: `falcon_cli/tests/cli_tests.rs`
   - Mock file system; test arg parsing
   - Verify exit codes

**Tests written:**
- Unit: clap parsing, arg validation
- Integration: invoke falcon binary with test args; verify output format

**Exit criteria:**
- `falcon --help` and `falcon check --help` work
- `falcon --version` outputs semver
...
**What gets built:**
1. `falcon_cli/src/file_walker.rs`
...
2. Tests: `falcon_cli/tests/walker_tests.rs`
...
**What gets built:**
1. `falcon_cli/src/analyze_pipeline.rs`
...
2. Tests: `falcon_cli/tests/pipeline_tests.rs`
...
**Exit criteria:**
- `falcon check .` works on jfit mobile lib
...
**Activities:**
- Manual test: `falcon check` on jfit mobile lib (no rules yet, just parser + infrastructure)
...
1. `falcon_rules/docs/rule_specs.md` — detailed spec for each of 60 rules
...
2. Golden test corpus: `falcon_rules/tests/corpus/{rule_name}/`
...
3. Copy upstream fixture files verbatim into `crates/falcon_rules/tests/corpus/{rule_name}/`:
   - Preserve original file names (`example.dart`, `good_example.dart`, `bad_example.dart`, etc.)
   - Keep upstream annotations intact as documentation; add a `// falcon-expect:` companion annotation in the falcon format (`/* expect: rule_name, line X, col Y */`) derived from the upstream annotations
   - Commit all copied fixtures to the repo as baseline
4. Document the annotation format mapping in `crates/falcon_rules/tests/corpus/FIXTURE_FORMAT.md`:
   - How upstream `// expect_lint:` maps to falcon `/* expect: rule_name, line X, col Y */`
...
- `FIXTURE_FORMAT.md` documents the upstream-to-falcon annotation mapping
- Fixture files are committed under `crates/falcon_rules/tests/corpus/`
...
- Golden corpus has falcon-format annotations (`/* expect: rule_name, line X, col Y */`) derived from upstream
...
- Functionality:
  ...
  - Invoke falcon on each corpus file; parse diagnostic output
...
- Usage: `cargo xtask validate-rules --corpus falcon_rules/tests/corpus/ --threshold 0.85`
- M4.0 deliverable set (updated):
  1. `falcon_rules/docs/rule_specs.md` — detailed specs for all 60 rules (existing)
...
- Acceptance:
  ...
  - CI job added: `cargo xtask validate-rules` runs on every PR touching falcon_rules
...
**What gets built:**
1. `falcon_analyze_macros/src/lib.rs` (if needed) — derive macros for rule boilerplate
...
2. `xtask/codegen/rule_template.rs` — generate rule stub
   ...
   - Generates: `falcon_rules/src/rules/avoid_dynamic.rs` with trait impl stub
3. Rule registry: `falcon_rules/src/lib.rs`
...
- Integration: rule runs in full falcon pipeline; diagnostics collected correctly
...
**What gets built:**
1. All 60 rules compiled into single `falcon_rules` crate
...
1. `.omc/docs/LSP_CACHING_DESIGN.md` — Cache invalidation strategy and incremental analysis model
   ...
   - File watcher integration: monitor falcon.json for changes; reload rules on change
...
**What gets built:**
1. `falcon_lsp/src/lib.rs` — LSP server skeleton
...
3. Tests: `falcon_lsp/tests/lsp_tests.rs`
...
**What gets built:**
1. VS Code extension (minimal): `extensions/falcon-vscode/`
   - Extension manifest (package.json)
   - Client code: launch LSP server, show diagnostics
   - Configuration: path to falcon binary
...
- falcon binary launches from VS Code
...
**Activities:**
- End-to-end test: VS Code running falcon LSP on jfit code
...
**Problem:** Nix integration is a hard acceptance criterion (spec: "jfit/flake.nix updated; falcon available in devShell"). If M7.2 fails in week 10-11, minimal recovery time remains.

**What gets built:**
1. Assign Nix engineer (dedicated, named in M10 team table) from week 1 — not just "review in M7"
2. In M0.5 (week 1): Nix engineer reviews both falcon/flake.nix design and jfit/flake.nix integration plan (2 hours)
...
3. Early validation gates:
   - M7.0 (week 1): `nix flake check` on draft falcon/flake.nix skeleton — verifies syntax
   - M7.1 (week 9): `nix build .#falcon` — verifies derivation produces binary
   - M7.2 (week 8, parallel to M5): Test falcon input in jfit flake — moved earlier
...
**Decision (Locked):**
- **Phase 1 uses `path:` URL** for local development iteration: `falcon.url = "path:/home/jacob/Documents/Developer/falcon"`
  - Rationale: Rapid iteration; allows jfit to pick up falcon changes immediately (no tag/release cycle needed during Phase 1)
  - Non-blocking: path: URL works for local flake.nix evaluation
- **Phase 2 will switch to `github:` URL with hash-pinning**: `falcon.url = "github:jacobsanderson/falcon/<tag>"`
...
#### M7.1: falcon Flake & Build Derivation (Days 57-58)

**What gets built:**
1. `flake.nix` at falcon repo root (already sketched in spec)
   - Build derivation: `buildRustPackage` for falcon binary
   - Inputs: nixpkgs, flake-utils, Rust stable
   - Outputs: falcon binary in default package
2. Tests:
   - `nix build .#falcon` produces binary at result/bin/falcon
   - Binary runs: `result/bin/falcon --version` outputs semver
...
**Exit criteria:**
- `nix build .` produces falcon binary
...
1. Update `/home/jacob/Documents/Developer/jfit/flake.nix`:
   - Add input: `falcon.url = "path:/home/jacob/Documents/Developer/falcon"` (Phase 1 strategy per M7.0)
   - Add package: `falcon = falcon.packages.${system}.default`
   - Add to devShell: `pkgs.falcon`, `falcon.packages.${system}.default`
   - Documentation: note that Phase 2 will switch to `github:` URL with hash-pinning
2. Tests:
   - `nix develop` in jfit; verify falcon on PATH
   - `falcon check apps/mobile/lib` runs in jfit devShell

**Tests written:**
- Integration: jfit flake.nix evaluates with falcon input (path: URL)
- Smoke test: falcon available in devShell

**Exit criteria:**
- jfit flake accepts falcon input (path: URL)
- falcon binary available in jfit devShell
- `nix develop` in jfit makes falcon available
...
**Activities:**
- Manual test: `nix develop` in jfit; run `falcon check` on jfit mobile lib
- Verify falcon runs, emits diagnostics, exits correctly
- Document jfit integration in README

**Exit criteria (gating M8):**
- falcon fully integrated into jfit flake
- falcon available in jfit devShell
- CI pipeline includes falcon check
...
**What gets built:**
1. CLI test suite: `falcon_cli/tests/acceptance_tests.rs`
   - `falcon check .` on jfit mobile lib
   - Verify exit code (1 if diagnostics found)
   - Verify JSON output format
   - Verify text output format
   - Test exclude patterns
   - Test config override flags
2. Performance test: measure `falcon check` on jfit (must be <1s)
...
**What gets built:**
1. LSP test suite: `falcon_lsp/tests/acceptance_tests.rs`
...
**What gets built:**
1. `README.md` — falcon overview, installation, usage
   - What is falcon? (Rust-based Dart linter)
   - Installation: `nix develop` in jfit or standalone `cargo build`
   - Usage: `falcon check .`, `falcon --config falcon.json`, LSP setup
   - Configuration: falcon.json schema with examples
   - Troubleshooting
...
- Source location: `extensions/falcon-vscode/` in falcon repo
- Phase 1 installation method:
  ```
  # Option A: Install from falcon repo (manual)
  git clone https://github.com/jacobdev/falcon
  cd falcon
  code --install-extension ./extensions/falcon-vscode/
  
  # Option B: Install from jfit devShell (automatic if bundled)
  nix develop  # jfit devShell
  # falcon-vscode bundled if configured in jfit devShell
  ```
...
**Step M0.1: Rename repo from falcon to falcon**
- Crate: core workspace
- Acceptance: `Cargo.toml` has `name = "falcon"`, all refs updated

**Step M0.2: Scaffold workspace Cargo.toml**
- File: `Cargo.toml`
- Content: `[workspace]` with members: falcon_dart_parser, falcon_syntax, falcon_analyze, falcon_rules, falcon_diagnostics, falcon_lsp, falcon_config, falcon_cli, falcon (binary)
...
- File: `.omc/plans/falcon-rules-matrix.md`
...
- Content: Dart 3.x grammar scope for falcon Phase 1 (see M0.5 milestone section above)
...
**Step M1.1.1: Create falcon_dart_parser crate**
- Crate: falcon_dart_parser
- File: `crates/falcon_dart_parser/Cargo.toml`
- Acceptance: crate compiles

**Step M1.1.2: Implement Lexer**
- Crate: falcon_dart_parser
- File: `crates/falcon_dart_parser/src/lexer.rs`
...
- File: `crates/falcon_dart_parser/tests/lexer_tests.rs`
...
- File: `crates/falcon_dart_parser/src/grammar.md`
...
- Crate: falcon_dart_parser
- File: `crates/falcon_dart_parser/src/parser.rs`
...
- File: `crates/falcon_dart_parser/src/parser.rs` (extended)
...
- File: `crates/falcon_dart_parser/tests/parser_tests.rs`
...
**Step M1.3.1: Create falcon_syntax crate**
- Crate: falcon_syntax
- File: `crates/falcon_syntax/Cargo.toml`
- Acceptance: crate compiles

**Step M1.3.2: Define AST node types**
- Crate: falcon_syntax
- File: `crates/falcon_syntax/src/ast.rs`
...
- File: `crates/falcon_syntax/src/syntax_kind.rs`
...
- File: `crates/falcon_syntax/src/visitor.rs`
...
- File: `crates/falcon_syntax/tests/ast_tests.rs`
...
- Crate: falcon_dart_parser
- File: `crates/falcon_dart_parser/tests/corpus_tests.rs`
...
- File: `crates/falcon_dart_parser/tests/snapshots/` (insta generated)
...
**Step M2.1.1: Create falcon_diagnostics crate**
- Crate: falcon_diagnostics
- File: `crates/falcon_diagnostics/Cargo.toml`
- Deps: serde, serde_json (for serialization)
- Acceptance: crate compiles

**Step M2.1.2: Define Diagnostic types**
- File: `crates/falcon_diagnostics/src/lib.rs`
...
- File: `crates/falcon_diagnostics/tests/diag_tests.rs`
...
**Step M2.2.1: Create falcon_analyze crate**
- Crate: falcon_analyze
- File: `crates/falcon_analyze/Cargo.toml`
- Deps: falcon_syntax, falcon_diagnostics, rayon
- Acceptance: crate compiles

**Step M2.2.2: Define Rule trait**
- File: `crates/falcon_analyze/src/rule.rs`
...
- File: `crates/falcon_analyze/src/visitor.rs`
- Trait: `RuleVisitor: Visitor<Vec<Diagnostic>>` (extends Visitor from falcon_syntax)
...
- File: `crates/falcon_analyze/src/context.rs`
- Struct: `AnalyzeContext { config: FalconConfig, file_path: PathBuf, source: String, diags: Vec<Diagnostic> }`
- Methods: add_diagnostic(), get_config(), get_line_col()
- Acceptance: context provides all info rules need

**Step M2.2.5: RuleRegistry**
- File: `crates/falcon_analyze/src/registry.rs`
...
- File: `crates/falcon_analyze/src/parallel.rs`
- Function: `analyze_parallel(rules: &RuleRegistry, files: Vec<(Path, Source)>, config: &FalconConfig) -> Vec<Diagnostic>`
...
- File: `crates/falcon_analyze/tests/analyze_tests.rs`
...
**Step M2.3.1: Create falcon_config crate**
- Crate: falcon_config
- File: `crates/falcon_config/Cargo.toml`
- Deps: serde, serde_json
- Acceptance: crate compiles

**Step M2.3.2: Define FalconConfig types**
- File: `crates/falcon_config/src/lib.rs`
- Struct: `FalconConfig { rules: HashMap<String, RuleConfig>, exclude_patterns: Vec<String>, severity_override: HashMap<String, Severity>, max_errors: Option<usize> }`
...
- File: `falcon.json` (in repo root)
- Content: all 60 Phase 1 rules enabled; example options for configurable rules
- Acceptance: falcon.json loads without error

**Step M2.3.4: Config loading**
- File: `crates/falcon_config/src/loader.rs`
- Function: `load_config(path: &Path) -> Result<FalconConfig, ConfigError>`
- Handles: missing file (use defaults), invalid JSON, missing fields (use defaults)
- Acceptance: can load falcon.json and default

**Step M2.3.5: Config tests**
- File: `crates/falcon_config/tests/config_tests.rs`
...
- Crate: falcon (or new falcon_tracing if separate)
...
- Files: Update falcon_dart_parser, falcon_analyze, falcon_lsp, falcon_cli with logging
...
- File: `crates/falcon_cli/src/args.rs`
- Content: Add --verbose flag; when set, initialize tracing with DEBUG level instead of INFO
- Acceptance: `falcon check . --verbose` shows DEBUG logs

**Step M2.4.4: Observability tests**
- File: `crates/falcon_cli/tests/logging_tests.rs`
...
**Step M3.1.1: Create falcon_cli crate & binary crate**
- Crates: falcon_cli, falcon
- Files: `crates/falcon_cli/Cargo.toml`, `crates/falcon/Cargo.toml`
- Deps: clap (derive), falcon_analyze, falcon_config
...
- Acceptance: `falcon --help`, `falcon check --help` work correctly

**Step M3.1.3: Main entry point**
- File: `crates/falcon/src/main.rs`
- Code: parse args, dispatch to subcommand, exit with code
- Acceptance: `falcon --version` outputs semver; `falcon check` with missing args shows help

**Step M3.1.4: CLI tests**
- File: `crates/falcon_cli/tests/cli_tests.rs`
...
- File: `crates/falcon_cli/src/file_walker.rs`
...
- File: `crates/falcon_cli/tests/walker_tests.rs`
...
- File: `crates/falcon_cli/src/pipeline.rs`
- Function: `run_check(path: &Path, config: &FalconConfig, parallel: bool) -> Result<CheckOutput, Error>`
...
- File: `crates/falcon_cli/src/output.rs`
...
- File: `crates/falcon_cli/tests/pipeline_tests.rs`
...
- Crate: falcon_rules
- File: `crates/falcon_rules/src/rules/avoid_dynamic.rs`
- Struct: `AvoidDynamic;`
...
- File: `crates/falcon_rules/src/rules/avoid_dynamic.rs` (extended)
...
- File: `crates/falcon_rules/src/rules/avoid_dynamic_tests.rs` or `tests/rules/avoid_dynamic_tests.rs`
...
- File: `crates/falcon_rules/tests/corpus/avoid_dynamic/`
- Files: upstream fixture `.dart` files copied verbatim from dart_code_linter/pyramid_lint test suites (via M4.0.1), with `/* expect: rule_name, line X, col Y */` falcon annotations added alongside original upstream annotations
...
Key files:
- `crates/falcon_lsp/src/lib.rs` — LSP server
- `crates/falcon_lsp/src/state.rs` — server state (cache, diagnostics)
- `crates/falcon_lsp/src/handlers.rs` — onOpen, onChange, onSave, publishDiagnostics
- `extensions/falcon-vscode/` — VS Code extension
...
- Activity: profile falcon on jfit corpus; identify bottlenecks
...
#### CLI + Config + Analyze
- **End-to-end check**: `falcon check jfit_sample/` with config file
...
#### Nix + CLI + Jfit
- **falcon check on jfit mobile lib**: full 214-file corpus
- **Nix build falcon**: produces binary
- **jfit devShell**: falcon available on PATH
...
- **Performance**: falcon check completes in <1000ms
...
**Title:** Biome-Inspired Modular Workspace for Dart Linter (falcon) Phase 1

**Status:** Accepted (Deliberate consensus, 2026-06-09)

### Decision

Build falcon as a **Biome-inspired modular Rust workspace** with 8-10 semantic crates:
- `falcon_dart_parser` (hand-rolled Dart 3.x recursive descent parser)
- `falcon_syntax` (AST node types, visitor trait)
- `falcon_analyze` (rule trait, registry, parallel execution)
- `falcon_diagnostics` (diagnostic types, serialization)
- `falcon_config` (falcon.json deserialization)
- `falcon_lsp` (Language Server Protocol server)
- `falcon_cli` (command-line interface, file walker, output formatting)
- `falcon` (binary entrypoint)
- `falcon_analyze_macros` (optional, for boilerplate reduction)
...
2. **Testability**: Rule logic is isolated in `falcon_rules`; rules are testable without full LSP or CLI stack.
...
3. **Testing Isolation**: Rules can be unit-tested in `falcon_rules` without instantiating CLI or LSP, enabling fast iteration.
4. **Performance**: Rayon parallelism in `falcon_analyze` provides <1s on jfit corpus; single-language focus means no cross-language coordination.
...
1. **Phase 2 Planning**: Design formatter pipeline; determine if new crates needed (falcon_dart_formatter, falcon_semantic_analysis)
2. **Rule Macro Evaluation**: M4.1 decides whether `falcon_analyze_macros` is justified; feedback will inform code generation strategy
...
3. **VS Code extension publishing**: Should falcon-vscode extension be published to VS Code Marketplace, or distributed as part of jfit toolchain? (Affects M5.3 and release strategy)
...
1. **Rust Workspace**
   ...
   - falcon.json config file (biome.json-style)
...
4. **Nix & Integration**
   - falcon/flake.nix (falcon build derivation)
   - Updated jfit/flake.nix (falcon as input; binary in devShell)
   - Binary available in jfit devShell via `nix develop`

5. **VS Code Extension**
   - Minimal extension (language client)
   - Connects to falcon LSP server
   - Displays diagnostics in editor

### Acceptance Criteria (All Must Pass)

- [x] Repo renamed to falcon; Cargo.toml name = "falcon"
...
- [x] falcon check . completes in <1000ms on jfit mobile project
- [x] LSP server starts and VS Code shows falcon diagnostics for .dart files
- [x] falcon.json config controls rule enable/disable
- [x] nix build produces falcon binary
- [x] jfit/flake.nix updated; falcon available in devShell
- [x] nix develop in jfit makes falcon available on PATH
...
|------|--------|-------|-------|-------|
| **Parser Engineer** (Agent 1) | falcon_dart_parser, falcon_syntax | 2-3 | 100-120 | M1, corpus tests, fuzzing (optional) |
| **Rule Engineers** (Agents 2-6, 5 total) | falcon_rules | 2-3 | 210-330 (parallel) | M4: 10 rules per agent, 35-55 hrs each |
| **Infrastructure Engineer** (Agent 7) | falcon_analyze, falcon_diagnostics, falcon_config | 2 | 80-100 | M2, rule trait design, test harness |
| **CLI/LSP Engineer** (Agent 8) | falcon_cli, falcon_lsp, VS Code extension | 2-3 | 80-100 | M3, M5, CLI, LSP, incremental analysis |
