...
|-------|---------------|-------|
| `falcon_dart_parser` | Dart 3.x lexer + recursive-descent parser → AST | M1 |
| `falcon_syntax` | AST node types, token types, syntax tree | M1 |
| `falcon_analyze` | `Rule` trait, `RuleVisitor`, `AnalyzeContext`, Rayon engine | M2 |
| `falcon_rules` | All ported lint rules (dart_code_linter + pyramid_lint) | M4 |
| `falcon_diagnostics` | `Diagnostic`, `Severity`, `Span` types | M2 |
| `falcon_config` | `falcon.json` schema, config loader | M3 |
| `falcon_cli` | CLI args (clap), `check` + `lsp` commands | M3 |
| `falcon_lsp` | LSP 3.17 server over JSON-RPC 2.0 | M5 |
| `falcon` (binary) | Entry point; wires CLI → analyze → diagnostics | M3 |
