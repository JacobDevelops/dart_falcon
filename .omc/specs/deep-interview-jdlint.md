# Deep Interview Spec: jdlint — Rust-Based Dart Linter

## Metadata
- Interview ID: jdlint-2026-06-09
- Rounds: 7
...
| CLI & Config | active | jdlint check/lint --fix CLI + biome.json-style config file | Config file format to resemble biome.json; covers rule enable/disable, project exclude patterns |
| Nix Flake | active | Package jdlint as Nix flake, add as input to jfit's flake.nix | Exposes binary + devShell overlay; added to `/home/jacob/Documents/Developer/jfit/flake.nix` |

## Goal
Build `jdlint` — a Rust-based Dart linter that replaces the `custom_lint` plugin layer (Phase 1) and eventually the full `dart analyze` pipeline (Phase 2). Phase 1 ships with a hand-rolled Dart 3.x parser, a Biome-inspired parallel rule engine, a full LSP server for editor integration, a biome.json-style config file, and ports all enabled rules from `dart_code_linter` and `pyramid_lint` as used in the jfit mobile project. It is packaged as a Nix flake and installed as an input to jfit's existing `flake.nix`.
...
- **Nix**: jdlint is a Nix workspace (Cargo workspace + Nix flake); binary exposed as a flake output and added as an input to `/home/jacob/Documents/Developer/jfit/flake.nix`
- **Directory**: Current repo at `/home/jacob/Documents/Developer/jlint` must be renamed to `jdlint`
...
## Acceptance Criteria
- [ ] Repository renamed from `jlint` to `jdlint` with updated Cargo.toml `[package] name = "jdlint"`
...
- [ ] `jdlint check .` completes in under 1 second on the jfit mobile project
- [ ] LSP server starts and VS Code (via generic LSP extension) shows jdlint diagnostics for a `.dart` file
- [ ] `jdlint.json` config at project root controls which rules are enabled/disabled; jfit's config file is committed
- [ ] `nix build` in the jdlint repo produces the `jdlint` binary
- [ ] `jfit/flake.nix` updated to include `jdlint` as an input and expose the binary in `devShell`
- [ ] `nix develop` in jfit makes `jdlint` available on PATH
...
| jfit has no flake.nix | Checked repo — flake.nix exists at `/home/jacob/Documents/Developer/jfit/flake.nix` | Add jdlint as an `inputs` entry to the existing flake |
...
- **jfit flake**: `/home/jacob/Documents/Developer/jfit/flake.nix` — existing Nix flake to receive jdlint as an input

### Current linters in jfit/apps/mobile
| Package | Version | Role | Port target |
|---------|---------|------|------------|
| `custom_lint` | ^0.8.1 | Plugin runner framework | Replaced entirely by jdlint |
...
├── flake.nix               # jdlint Nix flake
├── jdlint.json             # default config (checked into jfit later)
├── crates/
│   ├── jdlint_dart_parser/ # hand-rolled Dart 3.x recursive descent parser
│   ├── jdlint_syntax/      # AST node types, SyntaxKind enum
│   ├── jdlint_analyze/     # rule trait, visitor, query infrastructure
│   ├── jdlint_rules/       # all lint rule implementations
│   ├── jdlint_diagnostics/ # Diagnostic, Severity, Span types
│   ├── jdlint_lsp/         # LSP server (tower-lsp or lsp-server crate)
│   ├── jdlint_config/      # jdlint.json deserialization (serde_json)
│   ├── jdlint_cli/         # clap-based CLI: check, lint, lsp
...
| JdlintConfig | supporting | rules_config, exclude_patterns | loaded from jdlint.json |
| NixFlake | external system | inputs, devShell, packages | exposes jdlint binary to jfit |
...
### Round 3
**Q:** What exactly is jdlint replacing — full dart analyze, or just the custom_lint plugin layer?
...
### Round 4 (Contrarian)
**Q:** Does jdlint need to show diagnostics in your editor/IDE, or is CLI + CI the success target?
**A:** Full LSP server — editor integration required.
**Ambiguity:** 36.5% (Goal: 0.80, Constraints: 0.60, Criteria: 0.45)

### Round 5
**Q:** How slow is linting today, and what speed would make you say jdlint is fast enough?
...
### Round 7
**Q:** I found package.nix + shell.nix in jfit/apps/mobile but no flake.nix there. How should jdlint integrate with Nix?
**A:** It already exists at /home/jacob/Documents/Developer/jfit/flake.nix — add jdlint as an input to it.
